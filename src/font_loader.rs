...
pub fn get_glyph_table() -> HashMap<char, Glyph> {
    ...
    let font = fontdue::Font::from_bytes(font_bytes, fontdue::FontSettings::default())
        .expect("Failed to initialize font");
    let mut rasterized_glyphs: HashMap<char, Glyph> = HashMap::new();

    let chars = font.chars();

    for (ch, _) in chars {
        let (mut metrics, glyph_bytes) = font.rasterize(*ch, 24.0);
        if *ch == ' ' {
            metrics.width = (24.0 / 2.0) as usize
        };
...
pub fn get_max_glyph_height(glyph_map: &HashMap<char, Glyph>) -> usize {
    let tallest_glyph = glyph_map
        .values()
        .max_by_key(|x| x.metrics.height)
        .expect("getting tallest_glyph failed");
